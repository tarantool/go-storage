...
func TestPredicate_EncodeMsgpack(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Quirk: TCS errors when mod_revision references an absent key.
			// VersionEqual(key, 0) is the etcd "key absent" idiom, and the
			// encoder rewrites it to count == 0 on the wire. See the
			// EncodeMsgpack doc comment.
			name:        "version equal zero rewrites to count target",
			predicate:   goPredicate.VersionEqual([]byte("test-key"), int64(0)),
			expectError: false,
			expected: []byte{
				0x94, 0xa5, 0x63, 0x6f, 0x75, 0x6e, 0x74, 0xa2, 0x3d, 0x3d,
				0xd3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xa8,
				0x74, 0x65, 0x73, 0x74, 0x2d, 0x6b, 0x65, 0x79,
			},
		},
		{
			// Symmetric to above: VersionNotEqual(key, 0) rewrites to count != 0.
			name:        "version not equal zero rewrites to count target",
			predicate:   goPredicate.VersionNotEqual([]byte("test-key"), int64(0)),
			expectError: false,
			expected: []byte{
				0x94, 0xa5, 0x63, 0x6f, 0x75, 0x6e, 0x74, 0xa2, 0x21, 0x3d,
				0xd3, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xa8,
				0x74, 0x65, 0x73, 0x74, 0x2d, 0x6b, 0x65, 0x79,
			},
		},
		{
			// Boundary: only == and != against 0 are rewritten. Greater
			// stays on the mod_revision target.
			name:        "version greater zero stays on mod_revision",
			predicate:   goPredicate.VersionGreater([]byte("test-key"), int64(0)),
			expectError: false,
			expected: []byte{
				0x94, 0xac, 0x6d, 0x6f, 0x64, 0x5f, 0x72, 0x65, 0x76, 0x69,
				0x73, 0x69, 0x6f, 0x6e, 0xa1, 0x3e, 0xd3, 0x00, 0x00, 0x00,
				0x00, 0x00, 0x00, 0x00, 0x00, 0xa8, 0x74, 0x65, 0x73, 0x74,
				0x2d, 0x6b, 0x65, 0x79,
			},
		},
		{
			// Boundary: Less against 0 also stays on mod_revision.
			name:        "version less zero stays on mod_revision",
			predicate:   goPredicate.VersionLess([]byte("test-key"), int64(0)),
			expectError: false,
			expected: []byte{
				0x94, 0xac, 0x6d, 0x6f, 0x64, 0x5f, 0x72, 0x65, 0x76, 0x69,
				0x73, 0x69, 0x6f, 0x6e, 0xa1, 0x3c, 0xd3, 0x00, 0x00, 0x00,
				0x00, 0x00, 0x00, 0x00, 0x00, 0xa8, 0x74, 0x65, 0x73, 0x74,
				0x2d, 0x6b, 0x65, 0x79,
			},
		},

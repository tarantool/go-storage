...
// syncMap is a thread-safe structure that holds the
...
func (d *Driver) Watch(_ context.Context, key []byte, _ ...watch.Option) (<-chan watch.Event, func(), error) {
	ch, cancel := d.addWatcher(string(key))
...
func (d *Driver) addWatcher(key string) (chan watch.Event, func()) {
	d.data.watchMu.Lock()
	defer d.data.watchMu.Unlock()
...
func (d *Driver) notifyWatchers(key string) {
	d.data.watchMu.RLock()
	defer d.data.watchMu.RUnlock()

...
func testPrefix(t *testing.T, prefix string) []byte {
	t.Helper()
...
func TestDummyDriver_Watch_Recursive(t *testing.T) {
	...
	prefix := testPrefix(t, "watch-root")
...
func TestDummyDriver_DeleteByPrefix(t *testing.T) {
	t.Parallel()
	...
	basePrefix := "/test/delete-prefix-test/" + t.Name() + "/"
	...
	putResp, err := driver.Execute(ctx, nil, []operation.Operation{
		operation.Put(key1, value1),
		operation.Put(key2, value2),
		operation.Put(key3, value3),
		operation.Put(key4, value4),
	...
	require.NoError(t, err)
	assert.True(t, putResp.Succeeded)

	getResp, err := driver.Execute(ctx, nil, []operation.Operation{
		operation.Get(key1),
	}, nil)
	require.NoError(t, err)

	initialRevision := getResp.Results[0].Values[0].ModRevision

	deleteResp, err := driver.Execute(ctx, nil, []operation.Operation{
		operation.Delete([]byte(basePrefix)),
	}, nil)
	require.NoError(t, err)
	assert.True(t, deleteResp.Succeeded)

	require.Len(t, deleteResp.Results, 1, "Delete operation should return one result")
	assert.Len(t, deleteResp.Results[0].Values, 3, "Delete by prefix should return three deleted values")

	deletedKeys := make(map[string][]byte)
	for _, kv := range deleteResp.Results[0].Values {
		deletedKeys[string(kv.Key)] = kv.Value
	}

	assert.Equal(t, value1, deletedKeys[string(key1)], "Should have deleted value1 for key1")
	assert.Equal(t, value2, deletedKeys[string(key2)], "Should have deleted value2 for key2")
	assert.Equal(t, value3, deletedKeys[string(key3)], "Should have deleted value3 for key3")

	getAfterResp, err := driver.Execute(ctx, nil, []operation.Operation{
		operation.Get([]byte(basePrefix)),
	}, nil)
	require.NoError(t, err)
	assert.True(t, getAfterResp.Succeeded)
	assert.Empty(t, getAfterResp.Results[0].Values, "Get by prefix after delete should return no values")

	otherGetResp, err := driver.Execute(ctx, nil, []operation.Operation{
		operation.Get(key4),
	}, nil)
	require.NoError(t, err)
	assert.True(t, otherGetResp.Succeeded)
	require.Len(t, otherGetResp.Results[0].Values, 1, "Key with different prefix should still exist")
	assert.Equal(t, value4, otherGetResp.Results[0].Values[0].Value, "Other key's value should be unchanged")

	putNewResp, err := driver.Execute(ctx, nil, []operation.Operation{
		operation.Put(key1, []byte("new-value")),
	}, nil)
	require.NoError(t, err)
	assert.True(t, putNewResp.Succeeded)

	getNewResp, err := driver.Execute(ctx, nil, []operation.Operation{
		operation.Get(key1),
	}, nil)
	require.NoError(t, err)

	newRevision := getNewResp.Results[0].Values[0].ModRevision
	assert.Greater(t, newRevision, initialRevision, "Revision should have increased after delete operation")
}

func TestDummyDriver_PrefixResultOrdering(t *testing.T) {
	t.Parallel()
	...
	basePrefix := "/test/ordering-test/" + t.Name() + "/"

	key3 := []byte(basePrefix + "key3")
	key1 := []byte(basePrefix + "key1")
	key2 := []byte(basePrefix + "key2")
	keyA := []byte(basePrefix + "keyA")
	keyB := []byte(basePrefix + "keyB")
	key10 := []byte(basePrefix + "key10")

	allKeys := [][]byte{key1, key2, key3, keyA, keyB, key10}
	for i, key := range allKeys {
		_, err := driver.Execute(ctx, nil, []operation.Operation{
			operation.Put(key, []byte{byte('A' + i)}),
		}, nil)
		require.NoError(t, err)
	...
	require.Len(t, resp.Results, 1, "Should have one result")
	require.Len(t, resp.Results[0].Values, len(allKeys), "Should return all keys with prefix")
	...
	for i, kv := range resp.Results[0].Values {
		assert.Equal(t, expectedOrder[i], string(kv.Key),
			"Key at position %d should be %s, got %s", i, expectedOrder[i], string(kv.Key))
	}
	...
	require.Len(t, deleteResp.Results, 1, "Should have one delete result")
	require.Len(t, deleteResp.Results[0].Values, len(allKeys), "Should delete all keys")

	for i, kv := range deleteResp.Results[0].Values {
		assert.Equal(t, expectedOrder[i], string(kv.Key),
			"Deleted key at position %d should be %s, got %s", i, expectedOrder[i], string(kv.Key))
	}
}
